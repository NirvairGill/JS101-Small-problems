...
/* On line 44 and 45, because of `+` operator implicitly coerces an array into string. We will get 
string value as an output not an array. On line 44 and 45, the method `join()` has apllied to string values. 
Since `join()` method can only be applied to an Array not String. We will always get error if applied to `string` value.
This programme demonstrats the implicit conversion of `+` operator in Javascript and it's Side Effects.
To fix it, we can use `Array.prototype.concat()` method.
```javascript
let dishName = random(adjective).concat(random(firstNoun).concat(random(secondNoun)));
let dish = random(ingredients, 3).concat(random(spices, 2).concat(random(extras, 1)));
```
