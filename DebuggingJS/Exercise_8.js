...
/* On line 11, we've defined `.sort()` method to the passing array. The `sort()` method
sorts the array starting from lowest first digit to highest first digits. If first digit is same,
then it compares the second digits and so on. For example, [10, 2, 1] will be sorted [1, 10, 2] 
with sort. This is exactly what's happening with some of the inputs. 
To fix it, we have to pass a callback function into `sort()` method and sort the array by its
numerical value.
*/